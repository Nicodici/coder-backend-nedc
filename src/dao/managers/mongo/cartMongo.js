...
export class CartMongo {
  ...
  addProductInCart = async (cid, product) => {
    try {
      const filter = { _id: cid, "products._id": product._id };
      const cart = await this.model.findById(cid);
      const findProduct = cart.products.some(
        (product) => product._id.toString() === product._id
      );
      ...
      if (findProduct) {
        ...
        const update = {
          $push: { products: { _id: product._id, quantity: product.quantity } },
        };
  ...
  deleteCartbyCid = async (cid) => {
    try {
      await this.model.deleteOne({ _id: cid });
      return res.send("carrito eliminado");
    } catch (error) {
      return res.send("Error al eliminar el carrito");
    }
  };
}
